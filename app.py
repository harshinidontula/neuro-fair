...
def load_data(file):
    ...
    try:
        if name.endswith('.csv') or name.endswith('.txt'):
            ...
            st.error(f"Unsupported file format for tabular ML Data: {name}. AI bias learning requires structured data (like CSV or Excel).")
            return None
...
menu = st.sidebar.radio("Navigation", ["1. Project Overview", "2. Data Upload", "3. Bias Detection", "4. Model Training & Fairness", "5. Bias Correction & Explainability", "6. Text Bias Detection (AI)"])
...
if menu == "1. Project Overview":
    ...
    """)
    st.image("https://images.unsplash.com/photo-1551288049-bebda4e38f71?ixlib=rb-4.0.3&auto=format&fit=crop&w=1000&q=80", caption="Fairness in Decision Making", use_container_width=True)

# --- Page 2: Data Upload ---
elif menu == "2. Data Upload":
    st.title("Data Upload")
    uploaded_file = st.file_uploader("Upload your dataset", type=["csv", "xlsx", "xls", "json", "txt", "docx", "pdf", "jpg", "jpeg", "png"], accept_multiple_files=False)
    ...
    if uploaded_file is not None:
        ...
        if is_image:
            ...
            try:
                ...
                if not text.strip():
                    st.error("❌ We couldn't extract any text from this PDF. It appears to be a scanned image or protected document. The AI requires machine-readable text.")
                else:
                ...
                st.error("PyPDF2 is not installed. Please run `pip install PyPDF2` from your terminal.")
            except Exception as e:
...
# --- Page 3: Bias Detection ---
    ...
    if df is not None:
        sensitive_col = st.selectbox("Select Sensitive Attribute", df.columns, index=list(df.columns).index("Gender") if "Gender" in df.columns else 0)
        target_col = st.selectbox("Select Target Variable", df.columns, index=list(df.columns).index("Approved") if "Approved" in df.columns else len(df.columns)-1)
        ...
        with col1:
            ...
            # Dynamic colors to support many categories
        ...
        with col2:
            ...
            # Rotate x-labels so long text names (like Authors) do not overlap
        ...
        if st.session_state.get('unstructured_text'):
            st.warning("You have uploaded an unstructured text document (PDF). Please navigate to **Page 6: Text Bias Detection (AI)** from the sidebar to analyze it! This page is strictly for tabular numerical datasets (CSV/Excel).")
        else:
...
# --- Page 4: Model Training ---
    ...
    if df is not None:
        ...
        # Prepare Data - auto-detect features (dropping non-numeric or encoding them)
        df_encoded = pd.get_dummies(df, drop_first=True)
        # Assuming the original target might have changed name
        ...
        if target_encoded_cols:
            ...
            st.session_state.baseline_metrics = {
                'acc': acc, 'sr_dict': sr_dict, 'dpd': dpd, 'features': features, 'importance': importance, 'true_target': true_target
            }
...
# --- Page 5: Correction & Explainability ---
    ...
    if df is not None and 'baseline_metrics' in st.session_state:
        ...
        # Identify sensitive features in encoded dataset
        ...
        if st.button("Apply Mitigation & Re-train"):
            ...
            for g in groups[:2]: # Show up to 2 groups in table
            ...
            st.success("Notice how the Selection Rates have ideally moved closer together, dropping Demographic Parity Difference. There's often a small trade-off in Accuracy.")
            ...
            importance_df = pd.DataFrame({'Feature': mitigated_features, 'Importance': m_imp}).sort_values(by='Importance', ascending=False)
...
# --- Page 6: Text Bias Detection ---
    ...
    user_text = st.text_area("Enter text to analyze:", value=default_text, height=200, placeholder="Paste your document or paragraph here...", label_visibility="collapsed")
...
{user_text}"""
                    ...
                    # Clean up JSON formatting if it includes ```json markdown blocks
                    ...
                    st.error("Failed to parse the AI output. The model returned an invalid JSON.")
